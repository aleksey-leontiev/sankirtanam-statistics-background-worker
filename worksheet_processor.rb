...
module WorksheetProcessor
  # processes file using specified path
  def WorksheetProcessor.process_file(path)
    result = []                  # result hash
  ...
  def WorksheetProcessor.get_version(sheet)
    begin
  ...
  def WorksheetProcessor.get_metadata(sheet, version, path)
    case version
    when 1
  ...
  def WorksheetProcessor.get_data(sheet, version)
    result = []
    start  = version == 2 ? 4 : 2
  ...
  def WorksheetProcessor.open_workbook(path)
